window.addEventListener('load', async (e) => {
    ...
    const topRated = cryptos.coins.find(coin => coin.rank === 1);
    dispayTopRated(topRated);


    // Pick two random coins and display them below
    const highRank = cryptos.coins.filter(coin => coin.rank <= 2);
    displayRankedCoin(highRank);

    // List all this coins
    listCoins(cryptos.coins);
});
...
function listCoins(coin) {
    coin.forEach(c => {
        ...
        coinMC.innerHTML = `1d chg <span class="${c.priceChange1d < 0.0 ? 'bearish' : 'bullish'}">${c.priceChange1d}%</span>`;
        pName.innerHTML = `<img src="${c.icon}" alt="${c.name}" /> <span>${c.name}</span>`;
        pMaxSup.innerHTML = `<span>Max Supply</span><span>${parseInt(c.totalSupply).toFixed(0)}</span>`;
        pAvSup.innerHTML = `<span>Av. Supply</span><span>${parseInt(c.availableSupply).toFixed(0)}</span>`;
        pMk.innerHTML = `<span>Mk. Cap.</span><span>${c.marketCap}</span>`;
        _1dp.innerHTML = `<span>1d Price</span><span class="${c.priceChange1d < 0.0 ? 'bearish' : 'bullish'}">${c.priceChange1d}%</span>`;
        exp.href = c.exp[0];
